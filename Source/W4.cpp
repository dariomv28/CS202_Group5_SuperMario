...
W4::W4(PlayerManager* player, sf::RenderWindow* window) :
    LevelManager(player, window)
{
    std::ifstream saveGame("SaveGame.txt");
    int currentWorld, currentLevel;
    saveGame >> currentWorld;
    saveGame >> currentLevel;
    std::cerr << currentWorld << " " << currentLevel << "\n";

    mapManager->convert_sketch(currentWorld, currentLevel, Enemies, Blocks, PowerUps, player);
...
void W4::render(sf::RenderTarget* target) {
    if (!target) {
        target = window;
    }
